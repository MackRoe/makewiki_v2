...
class PageListViewTests(TestCase):
    def test_multiple_pages_on_homepage(self):
...
class PageAddcardViewTests(TestCase):
    def test_page(self):
        # Make some test data to be displayed on the page.
        user = User.objects.create()
        ...
        # Issue a GET request to the MakeWiki homepage.
        # When we make a request, we get a response back.
        response = self.client.get('/addcard/')

        # Check that the response is 200 OK.
        self.assertEqual(response.status_code, 200)


class NewPageMadeTests(TestCase):
    def test_form_works(self):
        user = User.objects.create()

        # Page.objects.create(title="Fresh", content="Bleh! I look fresh!")

        form_info = {
            self.title: "Fresh Off the Runway",
            self.author: user,
            self.content: "Bleh! I look like I'm fresh off the runway!"
        }

        response = self.client.post(form_info)
